...
function ghost_remove_wp_ver_css_js( $src ) {
if ( strpos( $src, 'ver=' ) )
$src = remove_query_arg( 'ver', $src );
return $src;
}
add_filter( 'style_loader_src', 'ghost_remove_wp_ver_css_js', 9999 );

add_filter( 'script_loader_src', 'ghost_remove_wp_ver_css_js', 9999 );
// remove injected CSS for recent comments widget
function ghost_remove_wp_widget_recent_comments_style() {
if ( has_filter( 'wp_head', 'wp_widget_recent_comments_style' ) ) {
remove_filter( 'wp_head', 'wp_widget_recent_comments_style' );
}
}
add_filter( 'wp_head', 'ghost_remove_wp_widget_recent_comments_style', 1 );
// clean up comment styles in the head
function ghost_remove_recent_comments_style() {
global $wp_widget_factory;
if (isset($wp_widget_factory->widgets['WP_Widget_Recent_Comments'])) {
remove_action( 'wp_head', array($wp_widget_factory->widgets['WP_Widget_Recent_Comments'], 'recent_comments_style') );
}
}
add_action('wp_head', 'ghost_remove_recent_comments_style', 1);
}
// Media cleanup

// clean up gallery output in wp
function ghost_gallery_style($css) {
return preg_replace("!<style type='text/css'>(.*?)</style>!s", '', $css);
}
add_filter('gallery_style', 'ghost_gallery_style');
...
return preg_replace('/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $content);
    ...
    function ghost_img_caption_shortcode ( $empty, $attr, $content ) {
    ...
    add_filter( 'img_caption_shortcode', 'ghost_img_caption_shortcode', 10, 3 );
    add_action('after_setup_theme', 'ghost_after_setup');